...
import { Calendar, User, LayoutDashboard, LogOut, Menu, X, Plus, Users, MapPin, Clock, List } from 'lucide-react';
                            ...
                            return (
                                <div key={r.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center hover:shadow-md transition-shadow">
                                    <div className="flex gap-4 items-center">
                                        <div className="bg-orange-100 p-3 rounded-lg text-primary font-bold text-center min-w-[60px]">
                                            <div className="text-xs uppercase">{new Date(r.date).toLocaleDateString('es-ES', { month: 'short' })}</div>
                                            <div className="text-xl">{new Date(r.date).getDate()}</div>
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-gray-800">{center?.name}</h4>
                                            <p className="text-sm text-gray-500">{r.startTime} - {r.endTime}</p>
                                            <span className={`text-xs px-2 py-0.5 rounded-full ${r.status === 'CONFIRMED' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 text-center">
                    ...
                    <div className="text-sm text-gray-600 space-y-1 bg-gray-50 p-3 rounded-lg">
                        <p><span className="font-bold">Admin:</span> admin@misport.com</p>
