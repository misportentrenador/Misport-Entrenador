...
### Madurez estimada del sistema (2026-07-16, tras el Sprint 25): ~40%

Estimación cualitativa, no una métrica exacta: media ponderada del grado de completitud de cada uno de los 17 módulos del ERP (CRM ~90%, Agenda ~97%, Reservas ~92%, Catálogo ~90%, Bonos ~85%, **Finanzas ~65%** —sube del 60% al 65%: un único flujo de alta de sesiones/pagos (`PagoFormFields` compartido), sin la duplicidad que antes existía entre el "Registro" general de Finanzas y la vista por cliente, y que además dejaba entradas del "Registro" general invisibles para cualquier Ficha o factura—, Cuadros de mando ~50%, Facturación ~40%, Tesorería ~45%, Integraciones externas ~20%, IA ~5%, y Proyectos institucionales/Corporate Wellness/Trail Running/RRHH/Documentación/Automatizaciones en 0%). Movimiento marginal (40% se mantiene igual redondeando) — Sprint 25 es una consolidación técnica, no una funcionalidad nueva. Se recalculará al cierre de cada Sprint futuro con el mismo criterio, para que la cifra sea comparable entre informes.
...
Ninguno abierto. El **Sprint 25** (Consolidación de Finanzas) está implementado y probado: `PagoFormFields` (antes ya compartido entre la Ficha CRM y la Agenda) pasa a ser también el único formulario del "Registro" general de Finanzas (`FinancePanel`), que tenía su propia copia inline de los mismos campos más overrides manuales y notas que los otros dos sitios no tenían. Se resuelve en las dos direcciones: (a) el "Registro" general gana un selector de cliente opcional — antes, una sesión de un cliente real registrada ahí quedaba sin `personaId`, invisible en su Ficha y **nunca facturable**; (b) la Ficha CRM y el modal de Agenda ganan los overrides manuales de precio/pago y notas que antes solo existían en el "Registro" general. Sin cliente seleccionado, el "Registro" general se comporta exactamente igual que antes (sin `personaId`). Cero cambios de modelo de datos: todos los campos ya existían en `FinanceEntry` desde Sprints anteriores, solo estaban sin exponer en la UI compartida.
...
Reordenados el 2026-07-16 tras el cierre del Sprint 25, según el orden de prioridad vigente (1. completar flujos internos, 2. consolidar Agenda/CRM/Finanzas/Facturación, 3. finalizar acciones rápidas pendientes, 4. optimizar la experiencia diaria, 5. integraciones reales — deliberadamente en espera hasta consolidar el núcleo).
...
**Recomendación de secuencia**: Sprint 26 (Framework de pruebas automatizado versionado + auditoría de accesibilidad ampliada) — es el siguiente punto del ciclo de mantenimiento del 20% técnico; requiere una decisión tuya sobre herramienta de pruebas (Playwright Test como runner versionado, Vitest, u otra) antes de poder diseñarlo en firme.
