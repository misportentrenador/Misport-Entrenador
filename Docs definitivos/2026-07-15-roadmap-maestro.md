...
### Madurez estimada del sistema (2026-07-16, tras el Sprint 24): ~40%

Estimación cualitativa, no una métrica exacta: media ponderada del grado de completitud de cada uno de los 17 módulos del ERP (CRM ~90%, Agenda ~97%, Reservas ~92%, Catálogo ~90%, Bonos ~85%, Finanzas ~60%, Cuadros de mando ~50%, Facturación ~40%, **Tesorería ~45%** —sube del 25% al 45%: cobros parciales/fraccionados reales, forma de pago obligatoria, auditoría completa (usuario/fecha/hora) y cobro inmutable con identificador permanente; sigue faltando conciliación bancaria, remesas, facturación electrónica e integración bancaria real, que quedan preparadas en el modelo pero no implementadas—, Integraciones externas ~20%, IA ~5%, y Proyectos institucionales/Corporate Wellness/Trail Running/RRHH/Documentación/Automatizaciones en 0%). A diferencia de los últimos cierres, este movimiento (38% → 40%) no es marginal — Tesorería fase 2 es un avance real, no solo una acción rápida más. Se recalculará al cierre de cada Sprint futuro con el mismo criterio, para que la cifra sea comparable entre informes.
...
Ninguno abierto. El **Sprint 24** (Tesorería fase 2 — cobros parciales y formas de pago) está implementado y probado: una misma Factura admite ahora varios `CobroFactura` (parciales o completos), cada uno con método de pago obligatorio (`efectivo`/`transferencia`/`tarjeta`/`otro`), referencia opcional (nota de pago/banco) y auditoría (usuario/fecha/hora). El cobro es **inmutable** (no existe ninguna función para editarlo o borrarlo) y su `id` es el identificador único y permanente al que se enlazará en el futuro cualquier medio de cobro externo (conciliación bancaria, TPV, Bizum, Stripe, transferencia). No se introduce un nuevo `FacturaEstado`: `'emitida'` sigue significando "queda algo pendiente" (parcial o total), así que `personaTieneDeuda`, el panel "Cobros pendientes" y el filtro "Con deuda" del CRM siguen funcionando sin ninguna modificación — "cobrado parcialmente" es una etiqueta derivada en la UI, no un estado persistido. Emite un evento de dominio (`CobroRegistered`) al que el CRM ya se suscribe para el Historial de la Ficha, diseñado desde el principio para que Tesorería, KPIs, Contabilidad, un ERP externo, Automatizaciones o IA puedan suscribirse en el futuro sin tocar `registrarCobro`. También queda auditado quién generó cada Factura (`usuarioId`/`usuarioNombre`). Todos los campos nuevos son aditivos (sin migración de datos, mismo principio que `Reservation.asistencia?` del Sprint 23).
...
Reordenados el 2026-07-16 tras el cierre del Sprint 24, según el orden de prioridad vigente (1. completar flujos internos, 2. consolidar Agenda/CRM/Finanzas/Facturación, 3. finalizar acciones rápidas pendientes, 4. optimizar la experiencia diaria, 5. integraciones reales — deliberadamente en espera hasta consolidar el núcleo).
...
| 28 | **(en espera, arquitectura ya lista)** Conexión real de Google Calendar (activar credenciales) | 5 | Pospuesto por decisión estratégica hasta consolidar el núcleo — sin bloqueo técnico cuando se retome |
| 29 | **(en espera)** Segunda integración de calendario (Outlook o Apple Calendar) | 5 | Pospuesto — arquitectura (`CalendarSourceAdapter`) lista |
...
| 32 | **(en espera, arquitectura ya lista)** Tesorería fase 3: conciliación bancaria y remesas | 1 y 2 | Pospuesto — requiere decidir un proveedor/formato bancario; el modelo (`CobroFactura` inmutable con id permanente) ya está preparado |
| 33 | **(en espera)** Facturación electrónica (Facturae/Veri*Factu) | 1 y 2 | Pospuesto — depende de la normativa aplicable y de un proveedor/certificado, no solo de MISPORT OS |

**Recomendación de secuencia**: Sprint 25 (consolidación de Finanzas — unificar el "Registro" general con la vista por cliente) — es una revisión técnica sin bloqueos de negocio pendientes, y cierra una duplicidad de flujos que lleva arrastrándose desde los Sprints 6-10.
